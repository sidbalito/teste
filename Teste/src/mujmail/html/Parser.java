...
import mujmail.html.element.ScriptElement;
...
public class Parser {
    ...
    public Vector parse() {
        ...
        for (int i = 0; i < html.length(); ++i) {
            ...
            switch (c) {
                case '<':
                    if ( text.length() > 0 ) {
                    	if(hide)System.out.println("oculto");
                    	if(!hide)//*/
    ...
    private int processElement(final int i, final char[] htmlChars, final Vector elements) {
		...
		if(htmlChars[pos] == '!')isComment = true;
        if(isComment){
        	System.out.println("Comentário");
        	elementType = AElement.UNPAIR;
            while (pos < htmlChars.length) {
            	if((pos+2 >= htmlChars.length)) {
            		isComment = false;
            		return pos+2;
            	}
            	if(htmlChars[pos] == '-'){
	            	char[] str = new char[3];
	            	System.out.print(htmlChars[pos]);
	            	//System.out.println(new String(htmlChars, pos, 10));
					System.arraycopy(htmlChars, pos, str, 0, 3);
					System.out.println(pos+": "+new String(str) + " >> "+htmlChars.length);
					if("-->".equals(new String(str))) {
					//System.out.println(pos+": "+new String(htmlChars, pos, 130) + " >> "+htmlChars.length);
						System.out.println("Cooment: \n"+ new String(htmlChars, i, pos-i+3));
						isComment = false;
						return pos+2;
					}
            	}
        ...
        if (!isLetter(htmlChars[pos]) & htmlChars[pos] != '!') {
        ...
        if (element != null) {
            ...
            if("script".equals( element.getName() )){
				if(element.getType() == AElement.START) hide = true;
            	if(element.getType() == AElement.END) hide = false;
            	System.out.println(element.getName()+" hide: "+hide);
            }
...
//        return (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || c == '/';
    ...
    private AElement getElementForName( final String elementName ) {
        ...
        if ( "a".equals(lower) ) {
            ...
            return new UElement();
        } else if ( "script".equals(lower) ) {
            return new ScriptElement();
/*        } else if ( "comment".equals(lower) ) {
            return new ScriptElement();//*/
